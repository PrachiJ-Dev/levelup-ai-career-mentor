...
const floatingBadges = [
  "DistilBERT Resume Analysis",
  "DNN Skill Mapping",
  "LSTM Career Prediction",
  "GPT-2 Interview Gen"
]
...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Auto-fill credentials if user just registered
    ...
    if (savedEmail) {
      ...
      // Clear them so they don't persist on subsequent visits
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!res?.error) {
      toast.success('Logged in successfully!', {
        style: { background: '#111118', color: '#fff', border: '1px solid rgba(59,130,246,0.4)' },
      })
        ...
        <div className="relative z-10 text-center flex flex-col items-center">
          ...
          <div className="mt-16 flex flex-col gap-4 relative h-64 w-full max-w-xs">
            {floatingBadges.map((badge, i) => (
              ...
              >
                <div className="glass-card py-3 px-6 text-sm font-medium text-blue-200 border border-blue-500/20 text-center shadow-lg backdrop-blur-md">
                  {badge}
                </div>
              </motion.div>
            ))}
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 relative">
        ...
        >
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
              <label htmlFor="login-password" className="block text-sm font-medium text-slate-300 mb-1.5 ml-1">
                Password
              </label>
              <div className="relative">
                <input
            ...
            <button
              type="button"
              className="w-full flex items-center justify-center gap-2 py-3 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-colors text-slate-200"
